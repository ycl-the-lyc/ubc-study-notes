...
Exceptions causes interrupts.
Interrupts are used when there is an error, or it is time to let other programs run, for example, ```asm ecall```.

Traps are exceptions generated by software.
They have number 0 to 15.

Number 16 and beyond are exceptions generated by hardware.

To trigger trap 3, software can write `1` to `0xFF202110`.
That is called ACLINT.
...
=== Machine Timer
It counts up.
It has 64 bits, but we can only access 32 bits at a time.

To read the entire timer:
+ read high;
+ read low;
...
To read an entire timer, they have snapshots, so just read the snapshot instead of the timers.

To start a timer, write `1` to its start bit.
